...
import { PredictionAgentConfig, ProteinStructure, Atom } from '../src/types';
...
async function main() {
  ...
  console.log('-------------------------');
  const normalPredictor = new ByzantinePredictor({
    numAgents: 7,
    faultTolerance: 2,
  ...
  const customAgents: PredictionAgentConfig[] = [
  ...
  const byzantinePredictor = new ByzantinePredictor({
    numAgents: 7,
    faultTolerance: 2,
    consensusThreshold: 2/3,
    rmsdThreshold: 2.0
  ...
  const rmsd = calculateRMSD(
    normalResult.consensusStructure.atoms.filter(a => a.atomName === 'CA'),
    byzantineResult.consensusStructure.atoms.filter(a => a.atomName === 'CA')
  );
  ...
  await visualizer.exportComparison(
    normalResult.consensusStructure,
    byzantineResult.consensusStructure,
    './output/byzantine-comparison'
  );
 ...
 */
function calculateRMSD(atoms1: Atom[], atoms2: Atom[]): number {
