    ...
    switch (cmd) {
      ...
      case 'rules':
        return [
          ...
          '',
          '📊 *RUNNING RECORD (June 15–18 2026):*',
          ...
          '   June 18 picks (final — 4 qualified legs, 2 bets x 2 legs):',
          '     BET 1: Yankees ML (-154) + Goldschmidt over 0.5 RBIs (SGP, same game, correlated)',
          '            Yankees ~63% | Goldschmidt RBI ~67% | Combined ~47% | Target +140+',
          '            Burke 4.15 ERA + high walk rate = more baserunners = more RBI opportunities',
          '     BET 2: Mariners ML (-149) + Woo over 5.5 Ks (SGP, same game, correlated)',
          '            Woo: 36 Ks in last 3 home starts (19 IP, 0 ER). K prop ~70%. ML ~61%.',
          '            Positive correlation: Woo striking out batters = Mariners win.',
          '     REJECTED: Canada ML (group stage rule + neg EV), Embolo ATS (42%), Ndoye ATS (~24%)',
          '     INJURY: Julio Rodriguez OUT (hamstring spasm). Arozarena on IL. ML drops to ~61%.',
          '     KEY: Woo career-high 7 ER vs Orioles was ROAD at Camden Yards. Today = HOME.',
          '          Home ERA 2.37 (elite) vs Road ERA 5.93. K prop + ML correlation = valid SGP.',
          '   Confirmed: 6/6 bets won | ~13/13 legs hit | 1 zero-bet day (discipline ✅)',
          ...
          '📅 *CARRY FORWARD June 19:* Medvedev ML vs Altmaier (Halle QF, ~80%) — confirmed June 19',
        ].join('\n');
